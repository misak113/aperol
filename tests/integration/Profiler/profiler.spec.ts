...
describe('Profiler.profiler', function () {
	...
	let warnings: { message: string; action: Action }[];
	const onWarning = (message: string, action: Action) => {
	...
	it('should log when some saga takes too much time', async function () {
	...
	it('should log correct saga and action when more combined sagas', async function () {
		...
		should(warnings).lengthOf(2);
		should(warnings[1].action).eql({ type: A1 });
		should(warnings[0].action).eql({ type: A2 });
